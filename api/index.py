...
def get_yaml():
    global dir1
    with open(join(dir1, "..", "config.yaml"), mode='r') as file:
        crf = file.read()
...
def get_record(id1, csrf):
    header = {
        'User-Agent': config["ua"],
        'Cookie': config["cookie"],
        'Origin': "https://music.163.com",
        'Referer': "https://music.163.com/user/songs/rank?id=" + id1,
        }
    params = {
        'params': config["params"],
        'encSecKey': config["esk"]
    }
    response = requests.post("https://music.163.com/weapi/v1/play/record?csrf_token=" + csrf,
...
def drawer(ds, name, num=0):
    ...
    font = ImageFont.truetype(join(dir1, "..", "static", "Deng.ttf"), 60)
    text = "Favourite Songs of the Week" if num else "Favourite Songs of All Time"
    draw.text((10, 10), text, fill=(0, 0, 0), font=font)
    font = ImageFont.truetype(join(dir1, "..", "static", "Deng.ttf"), 20)
...
def week_post():
    ...
    json_record = get_record(id1, csrf=csrf)
    dataset = parse_record(json_record)
    drawer(ds=dataset, name=week, num=1)
    week.seek(0)
...
def all_time_post():
    ...
    json_record = get_record(id1, csrf=csrf)
    dataset = parse_record(json_record)
    drawer(ds=dataset, name=alltime, num=0)
    alltime.seek(0)
